...
/// Same as [RenderImage] with source.
...
/// A subpart of an image in the render tree.
///
...
class RenderSprite extends RenderBox {
  ...
  set source(Rect? value) {
    ...
    final needsLayout = value?.size != _source?.size;
    _source = value;
    if (needsLayout) {
      markNeedsLayout();
    } else {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final image = _image!;
    final opacity = _opacity?.value ?? 1.0;
    final scale = _scale;
    ...
    Size inputSize = Size(source.width, source.height);
    final fit = _fit ?? BoxFit.fill;
    ...
    paint.blendMode = colorBlendMode ?? BlendMode.srcOver;
    ...
    canvas.drawImageRect(image, source, destinationRect, paint);
    canvas.restore();
  }
